    ...
    describe('Test CRUD API methods', () => {
        ...
        it('should update user info', () => __awaiter(void 0, void 0, void 0, function* () {
            const res = yield request
                ...
                .set('Content-type', 'application/json')
                .set('Authorization', `Bearer ${token}`)
                .send(Object.assign(Object.assign({}, user), { user_name: 'muhamedTalaat', first_name: 'Muhammed', last_name: 'Talaat' }));
            expect(res.status).toBe(200);
            const { id, email, user_name, first_name, last_name } = res.body.data;
            expect(id).toBe(user.id);
            expect(email).toBe(user.email);
            expect(user_name).toBe('muhamedTalaat');
            expect(first_name).toBe('Muhammed');
            expect(last_name).toBe('Talaat');
        }));
        it('should delete user', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            expect(res.body.data.user_name).toBe('muhamedTalaat');
        }));
    });
});
